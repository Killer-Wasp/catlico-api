...
| JSON type | Generic `Column(JSON)`, **not JSONB** | Matches `enrichment.report` / `connector.manifest`. SQLite has no JSONB column type; generic `JSON` serializes to TEXT on SQLite, `json` on Postgres. We never query *inside* `details` in v1. |
