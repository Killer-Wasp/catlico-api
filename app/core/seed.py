...
async def seed_local_demo_data(session: AsyncSession) -> None:
    ...
    alert_specs = [
        AlertCreate(
            ...
            description="Encoded PowerShell command executed on SRV-FIN01. Command-line obfuscation consistent with Cobalt Strike beacon staging.",
            severity=3,
            tlp=2,
        ...
        AlertCreate(
            ...
            description="User reported email from 'Microsoft Support' with subject 'Password Expiry Notice'. Contains link to credential harvesting page.",
            severity=2,
            tlp=2,
        ...
        AlertCreate(
            ...
            description="Internal host 10.23.1.45 initiated connection to known Cobalt Strike C2 server. Traffic pattern matches beaconing behaviour over HTTPS.",
            severity=4,
            tlp=2,
        ...
        AlertCreate(
            ...
            description="Multiple failed login attempts against admin@example.com from IP 185.220.101.42. Source IP associated with known Tor exit node.",
            severity=2,
            tlp=2,
            date=now - timedelta(hours=6),
        ),
    ]
    for alert_in in alert_specs:
