...
async def test_create_user(session: AsyncSession):
    user = await create_user(session, UserCreate(email="test@example.com", password="secret"))
...
async def test_create_user_no_password(session: AsyncSession):
    user = await create_user(session, UserCreate(email="nopass@example.com"))
    assert user.hashed_password is None


async def test_get_user_by_email(session: AsyncSession):
    await create_user(session, UserCreate(email="find@example.com"))
...
async def test_get_user_by_email_case_insensitive(session: AsyncSession):
    await create_user(session, UserCreate(email="case@example.com"))
...
async def test_get_user_by_id(session: AsyncSession):
    created = await create_user(session, UserCreate(email="byid@example.com"))
...
async def test_get_users(session: AsyncSession):
    for i in range(3):
        await create_user(session, UserCreate(email=f"user{i}@example.com"))
...
async def test_get_users_pagination(session: AsyncSession):
    for i in range(5):
        await create_user(session, UserCreate(email=f"page{i}@example.com"))
    page = await get_users(session, skip=2, limit=2)
    assert len(page) == 2


async def test_update_user_email(session: AsyncSession):
    user = await create_user(session, UserCreate(email="old@example.com"))
...
async def test_update_user_email_normalized(session: AsyncSession):
    user = await create_user(session, UserCreate(email="norm@example.com"))
...
async def test_update_user_password(session: AsyncSession):
    user = await create_user(session, UserCreate(email="pw@example.com", password="old"))
...
async def test_update_user_superadmin(session: AsyncSession):
    user = await create_user(session, UserCreate(email="promote@example.com"))
    assert user.is_superadmin is False
...
async def test_authenticate_user_updates_last_login(session: AsyncSession):
    await create_user(session, UserCreate(email="login@example.com", password="pass"))
...
async def test_authenticate_user_wrong_password(session: AsyncSession):
    await create_user(session, UserCreate(email="wrong@example.com", password="secret"))
...
async def test_authenticate_user_no_password(session: AsyncSession):
    await create_user(session, UserCreate(email="nopass@example.com"))
